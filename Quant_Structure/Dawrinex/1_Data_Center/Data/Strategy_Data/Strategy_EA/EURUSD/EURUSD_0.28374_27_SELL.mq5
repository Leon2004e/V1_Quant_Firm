...
//|    Generated at 01/21/2026 09:52
//| 
//|    Backtested on EURUSD_DARWIN / M15,H4, 2018.01.01 - 2023.12.31 
//+------------------------------------------------------------------+
...
input string CustomComment = "Strategy 0.28374";
bool IndicatorLoadedWithoutError = true;
int regime_up = 0;        //regime_up
int regime_down = 0;        //regime_down
int vola_state = 0;        //vola_state
...
input int EMAPeriod2 = 150;        //EMAPeriod2
input int ATRPeriod1 = 100;        //ATRPeriod1
input int EMAPeriod3 = 39;        //EMAPeriod3
input int ATRPeriod2 = 13;        //ATRPeriod2
input double ProfitTarget1 = 70;        //ProfitTarget1
...
input string SignalTimeRangeFrom = "12:00";
...
#define ATR_1 3     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqATR", ATRPeriod1)
#define EMA_3 4     //iMA(NULL,0, EMAPeriod3, 0, MODE_EMA, PRICE_CLOSE)
#define ATR_2 5     //iCustom(NULL,0, "SqATR", ATRPeriod2)
#define ATR_3 6     //iCustom(NULL, 0, "SqATR", 20)
...
string sqStrategyName = "Strategy 0.28374";
...
//------------------------
  if(_sqIsBarOpen == true){
   ...
   if(  (NormalizeDouble((double) sqClose(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 1), 6) > NormalizeDouble((double) sqGetIndicatorValue(EMA_1, 0, 1), 6))
)
   {
      
      // Action #1
        regime_up = (int) AssignVrbVleEntMlt1;

   } else {
      // Action #1
        regime_up = (int) AssignVrbVleEntMlt2;
...
//------------------------
  if(_sqIsBarOpen == true){
   ...
   if(  (NormalizeDouble((double) sqGetIndicatorValue(EMA_2, 0, 1), 6) > NormalizeDouble((double) sqGetIndicatorValue(EMA_2, 0, 6), 6))
)
   {
      
      // Action #1
        regime_down = (int) AssignVrbVleEntMlt1;

   } else {
      // Action #1
        regime_down = (int) AssignVrbVleEntMlt2;
...
//------------------------
  if(_sqIsBarOpen == true){
   ...
   if(  (NormalizeDouble((double) sqGetIndicatorValue(TRUERANGE_1, 0, 1), 6) > NormalizeDouble((double) (1.8 * sqGetIndicatorValue(ATR_1, 0, 1)), 6))
)
   {
      
      // Action #1
        vola_state = (int) AssignVrbVleEntMlt1;

   } else {
      // Action #1
        vola_state = (int) AssignVrbVleEntMlt2;
...
//------------------------
   if (_sqIsBarOpen == true
      &&   ((NormalizeDouble((double) regime_up, 6) == NormalizeDouble((double) 1, 6))
      &&   indyCrossesBelow("sqClose(NULL,0, 1)", "(sqGetIndicatorValue(EMA_3, 0, 1) + (sqGetIndicatorValue(ATR_2, 0, 1) * 0.1))", 0, 0, 0, 0))
)
   {
      // Action #1
        // Enter at Market
      openPrice = 0;  //Open at current market price
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL, openPrice, 2, StopLossCoef1 * sqGetIndicatorValue(ATR_3, 1)), "Current");
...
double sqGetExpressionByIdentification(string id, int shift) {
      ...
      if(id == "(sqGetIndicatorValue(EMA_3, 0, 1) + (sqGetIndicatorValue(ATR_2, 0, 1) * 0.1))") {
          return   (sqGetIndicatorValue(EMA_3, 0, shift + 1) + (sqGetIndicatorValue(ATR_2, 0, shift + 1) * 0.1));
      }
