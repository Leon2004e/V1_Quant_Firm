# -*- coding: utf-8 -*-
"""
Quant_Structure/FTMO/Data_Center/Data_Operations/Trades/Trade_Logger_Loop/Trade_Logger_FTMO_LIVE_<login>.py
...
# Robusteres Nachladen für Positionen:
# Wenn neue Exit-Deals gefunden werden, lade Historie rückwirkend für diese Positionen
...
class MT5ClosedTradeLogger:
    ...
    def poll_once(self) -> int:
        ...
        if recent_exit_pos_ids:
            # Robuste Variante:
            # komplette Historie für großen Rückblick laden und nur betroffene position_ids extrahieren
        ...
        inserted = self._append_new_closed(rebuilt_df)
        ...
        while True:
            ...
            try:
                ...
                if added == 0:
                    ...
                    print(
                        f"[LOOP] {loop_ts} | db_changed={added} | "
                        f"account_type={account_type} | server={server} | "
                        f"max_close_time_utc={max_close}"
                    )
            except Exception as e:
